	...
	foreach($trackNames as $trackName)
	{
		...
		foreach($carPrefixs as $carPrefix)
		{
			$query = "SELECT `total_rank` As `myTotal`,
					...
					FROM `drive_lfss`.`stats_rank_driver` WHERE `track_prefix`='$trackName' AND `car_prefix`='$carPrefix' AND `licence_name`='$licenceName'";
			$result = mysql_query($query,$link);
			if (!$result) {die(mysql_error());}
			if($row = mysql_fetch_array($result))
			{
				mysql_query("UPDATE `drive_lfss`.`stats_rank_driver` SET `position`='".$row[1]."' WHERE `track_prefix`='$trackName' AND `car_prefix`='$carPrefix' AND `licence_name`='$licenceName'",$link);
			}
		}
	}
}

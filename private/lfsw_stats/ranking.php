...
$result = mysql_query("SELECT `licence_name` FROM `driver`",$link);
if (!$result) {die(mysql_error());}
$licenceNames = array();
while ($row = mysql_fetch_array($result)) 
...
foreach($trackNames as $trackName)
{
	...
	foreach($carPrefixs as $carPrefix)
	{
		...
		foreach($licenceNames as $licenceName)
		{
			...
			FROM `lap_data`,`race_data`
			WHERE `lap_data`.`licence_name`='".addslashes($licenceName)."' 
			...
			//var_dump($row);
			$driverBest = $row[0];
			$driverBestS = (int)(( ($bestEver * $bestEver) * 5000 / ($driverBest*$driverBest) ));
			echo "$licenceName Best: ".$driverBest." | ".$driverBestS."\n";
			...
			//var_dump($row);
			$driverAverage = $row[0];
			$driverAverageS = (int)(( ($average * $average) * 5000 / ($driverAverage*$driverAverage) ));
			$driverStabilityS = (int)(( ($driverBest * $driverBest) * 5000 / ($driverAverage*$driverAverage) ));

			echo "$licenceName Average: ".$driverAverage." | ".$driverAverageS."\n";
			echo "$licenceName Stability: ".$driverStabilityS."\n";
			...
			echo "$licenceName Win 1(+3),2(+2) or 3(+1): ".$driverWin." | ".$driverWinS."\n";
			
			$rank = (int)($driverBestS + $driverAverageS+ $driverStabilityS + $driverWinS);
			
			$driverWinS = (int)$driverWinS;
			$result = mysql_query("DELETE FROM `drive_lfss`.`driver_rank` WHERE `licence_name`='$licenceName' AND `track_prefix`='$trackName' AND `car_prefix`='$carPrefix'",$link);
			if (!$result) {die(mysql_error());}
			$result = mysql_query("INSERT INTO `drive_lfss`.`driver_rank` VALUES('$licenceName','$trackName','$carPrefix','$driverBestS','$driverAverageS','$driverStabilityS','$driverWinS')",$link);
			if (!$result) {die(mysql_error());}
			

			echo "$licenceName Rank: $rank\n";
